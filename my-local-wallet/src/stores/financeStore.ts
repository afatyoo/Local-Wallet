...
export const useFinanceStore = create<FinanceState>((set, get) => ({
  ...
  addSaving: async (userId, data) => {
    ...
    const { data: result } = await savingsApi.create(apiData);
    if (result) {
      set((state) => ({ savings: [...state.savings, convertToFrontend.saving(result)] }));
    }
  },
  ...
  updateSaving: async (id, data) => {
    ...
    const { data: result } = await savingsApi.update(id, updateData);
    if (result) {
      set((state) => ({
        savings: state.savings.map((s) => (s.id === id ? convertToFrontend.saving(result) : s)),
      }));
    }
  },
